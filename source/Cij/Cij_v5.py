...
wil_import = np.load(r"%s\output\%s\wil_tot(z,i).npy" %(path, WF_folder))
def wil_tot(z,i):
...
wig_import = np.load(r"%s\output\base_functions_v2\wig(z,i).npy" %(path, WF_folder))
def wig(z,i):
    # it's i+1: first column is for the redshift array
...
def compute():
    ...
    #         for ell in ell_values:
    #             print("k = %i" %k)
    #             C_LL_array[i,j,k] = Cij_LL_function(i,j,ell)
    #             C_LG_array[i,j,k] = Cij_LG_function(i,j,ell)
    #             C_GG_array[i,j,k] = Cij_GG_function(i,j,ell)
    #             k = k+1
    
    return(C_LL_array, C_LG_array, C_GG_array)

def save():
    np.save(r"%s\output\Cijs_v3\Cij_LL.npy" %path, C_LL_array)
    np.save(r"%s\output\Cijs_v3\Cij_LG.npy" %path, C_LG_array)
    np.save(r"%s\output\Cijs_v3\Cij_GG.npy" %path, C_GG_array)
