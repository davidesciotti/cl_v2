...
WF_input_folder = "WFs_v10_noBias_nz300"
Cij_output_folder = "Cijs_v14_newBias_nz300"
...
wil_import = np.genfromtxt("%s/output/WF/%s/wil_tot.txt" %(path, WF_input_folder))
def wil_tot(z,i):
    ...
    return result
# wil_import = np.genfromtxt("%s/output/WF/%s/wil.txt" %(path, WF_input_folder))
# def wil(z,i):
#     # it's i+1: first column is for the redshift array
#     wil_interp = interp1d(wil_import[:,0], wil_import[:,i+1], kind = "linear")
#     result_array = wil_interp(z) #z is considered as an array
...
# here wig -> wig_nobias
wig_import = np.genfromtxt("%s/output/WF/%s/wig_noBias.txt" %(path, WF_input_folder))
def wig_noBias(z,i):
    # it's i+1: first column is for the redshift array
...
def sum_Cij_LG(i,j,ell):
    ...
    for nbin in range(zbins):
        ...
        print(result, z_minus[nbin], z_plus[nbin], i, j, b[nbin])
    return result
...
def reshape(array, name):
    ...
    # saving it
    np.savetxt("%s/output/Cijs_v13_newell_IA_newBias/%s.txt" %(path, name), output_2D)
