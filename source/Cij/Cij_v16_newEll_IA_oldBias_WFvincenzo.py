...
Cij_output_folder = "Cijs_v16_oldBias_WFvincenzo"
...
wil_import = np.genfromtxt("%s/data/everyones_WFs/%s/wil_vincenzo_8K_IA_noSSC.txt" %(path_SSC, WF_input_folder))
def wil_tot(z,i):
    ...
    return result
# wil_import = np.genfromtxt("%s/data/everyones_WFs/%s/wil.txt" %(path, WF_input_folder))
# def wil(z,i):
#     # it's i+1: first column is for the redshift array
#     wil_interp = interp1d(wil_import[:,0], wil_import[:,i+1], kind = "linear")
#     result_array = wil_interp(z) #z is considered as an array
...
# here wig -> wig_nobias
wig_import = np.genfromtxt("%s/data/everyones_WFs/%s/wig_vincenzo_8K_noSSC.txt" %(path_SSC, WF_input_folder))
def wig(z,i):
    # it's i+1: first column is for the redshift array
    ...
    result = result_array.item() #otherwise it would be a 0d array 
    return result
# wig_noBias_import = np.genfromtxt("%s/data/everyones_WFs/%s/wig_vincenzo_8K_noBias_noSSC.txt" %(path_SSC, WF_input_folder))
# def wig_noBias(z,i):
#     # it's i+1: first column is for the redshift array
#     wig_interp = interp1d(wig_noBias_import[:,0], wig_noBias_import[:,i+1], kind = "linear")
#     result_array = wig_interp(z) #z is considered as an array
#     result = result_array.item() #otherwise it would be a 0d array 
#     return result
...
# # integrand
# def K_ij_XC(z,i,j):
#     return wil_tot(z,j) * wig_noBias(z,i) / (E(z)*r(z)**2)
# def K_ij_GG(z,i,j):
