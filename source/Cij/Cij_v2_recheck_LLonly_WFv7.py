...
# wil_import = np.load(r"%s\output\night_computation_zColumn\wil_tot.npy" %path)
wil_import = np.genfromtxt(r"%s\output\WFs_v7_zcut_noNormalization\wil_tot.txt" %path)
...
# wig_import = np.load(r"%s\output\night_computation_zColumn\wig.npy" %path)
wig_import = np.genfromtxt(r"%s\output\WFs_v7_zcut_noNormalization\wig.txt" %path)
def wig(z,i):
    # it's i+1: first column is for the redshift array
...
def compute():
    C_LL = np.zeros((nbl, zbins, zbins))
    # C_GL = np.zeros((zbins, zbins, nbl))
    # C_GG = np.zeros((zbins, zbins, nbl)) 
    for ell in range(nbl):  #xxx that's the error!
        print("ell = ", ell)
        for i in range(zbins):
            for j in range(zbins):
                C_LL[ell, i,j] = Cij_LL(i,j,ell)
...
Cij_folder = "Cijs_v2_recheck\with_WF_v7"
C_LL = compute()
save()
