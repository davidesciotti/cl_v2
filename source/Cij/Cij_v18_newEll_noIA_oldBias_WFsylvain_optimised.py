...
Cij_output_folder = "Cijs_v18_oldBias_WFsylvain_speedtest"
...
wig_import = np.genfromtxt("%s/data/everyones_WFs/%s/wig_sylvain_noSSC.txt" %(path_SSC, WF_input_folder))
def wig(z,i): # old Bias
    # it's i+1: first column is for the redshift array
    wig_interp = interp1d(wig_import[:,0], wig_import[:,i+1], kind = "linear")
    result_array = wig_interp(z) #z is considered as an array
    result = result_array.item() #otherwise it would be a 0d array 
    return result
# wig_noBias_import = np.genfromtxt("%s/data/everyones_WFs/%s/wig_sylvain_noBias_noSSC.txt" %(path_SSC, WF_input_folder))
# def wig_noBias(z,i): # new Bias
#     # it's i+1: first column is for the redshift array
