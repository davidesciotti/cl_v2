...
path_WF  = "C:/Users/dscio/Documents/Lavoro/Programmi/SSC_comparison/data/everyones_WFs/WF_REORGANIZED_MASTERFOLDER_v2"
# xxx ho cambiato wil_tot in wil
wil_import = np.genfromtxt("%s/davide/nz300/wil_davide_IA_IST_nz300.txt" %(path_WF))
def wil_tot(z,i):
...
wig_import = np.genfromtxt("%s/davide/nz300/wig_davide_IST_nz300.txt" %(path_WF))
def wig(z,i):
    # it's i+1: first column is for the redshift array
...
def save():
    np.save(r"%s/output/Cij/%s/Cij_LL.npy" %(path, Cij_folder), C_LL_array)
    # np.save(r"%s/output/Cij/%s/Cij_LG.npy" %(path, Cij_folder), C_LG_array)
    # np.save(r"%s/output/Cij/%s/Cij_GG.npy" %(path, Cij_folder), C_GG_array)
################################################### end of function declaration

# C_LL_array, C_LG_array, C_GG_array = compute()
C_LL_array = compute() # to spare time
