...
def wil_tilde(z, i): #attento, rivedi bene
...
column = 0
WF = np.asarray([wil_tot(zi, column) for zi in z])
plt.plot(z, WF, "r", label = "davide")

wil_vincenzo = np.genfromtxt("%s/data/dati_vincenzo/WiL.dat" %path)
plt.plot(wil_vincenzo[:,0], wil_vincenzo[:,1], "b", label = "vincenzo")
plt.yscale("log")
plt.legend()
plt.title("")
#
np.savetxt("%s/output/wil_tot(zi,%i).txt" %(path, column), WF)

# wig
#column = 0
#vals = np.asarray([wig(zi, column) for zi in z])
#plt.plot(z, vals, "r", label = "davide")
#
#wig_vincenzo = np.genfromtxt("%s/data/dati_vincenzo/WiG.dat" %path)
#plt.plot(wig_vincenzo[:,0], wig_vincenzo[:,1], "b", label = "vincenzo")
#plt.yscale("log")
#plt.legend()
#
#np.savetxt("%s/output/wig(zi,%i).txt" %(path, column), vals)
