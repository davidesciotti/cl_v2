...
ni_z_array_toSave[:,0] = z
for column in range(10):
    ni_z_array = np.asarray([n_i(zi, column) for zi in z])       
    ni_z_array_toSave[:, column+1] = ni_z_array 

np.save("%s/output/n_i(z).npy" %path, ni_z_array_toSave)
