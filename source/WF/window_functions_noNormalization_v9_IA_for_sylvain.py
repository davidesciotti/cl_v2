...
with open("options.txt", "w") as text_file:
    print("with zcut \n with nbar normalization \n with bias = 1", file=text_file)
...
# n_i_import = np.load(r"%s\output\%s\niz.npy" %(path, WFs_input_folder)) # davide ncutVincenzo
n_i_import = np.genfromtxt(r"%s\output\%s\niz.txt" %(path, WFs_input_folder)) # davide zcutVincenzo
def n_i(z,i):
...
def save(array, name):
    np.savetxt(r"%s\output\%s\%s.txt" %(path, WFs_output_folder, name), array)
...
# np.savetxt(r"%s\output\%s\L_ratio.txt" %(path, WFs_output_folder), array_appoggio)
# print("L_ratio DONE")
# array_appoggio = compute_2D(n_i, z)
# np.savetxt(r"%s\output\%s\niz_e-19cut.txt" %(path, WFs_output_folder), array_appoggio)
# print("niz DONE")
...
# TODO IA DEBUGGING
# wil_davide = np.genfromtxt(r"%s\output\%s\wil.txt" %(path, WFs_output_folder))
# wil_tot_davide = np.genfromtxt(r"%s\output\%s\wil_tot.txt" %(path, WFs_output_folder))
