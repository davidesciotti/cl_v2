...
#niz_vincenzo_interp = np.asarray([n_i_vincenzo(zi, i) for zi in z])
#pph_array  = np.asarray([pph(zi, zp) for zi in z])
#nz_array   = np.asarray([n(zi) for zi in z])
#wil_SSC_davide = np.asarray([wil_SSC(zi, i) for zi in z])
...
wil_tot_davide = np.asarray([wil_tot(zi, i) for zi in z])
...
#plt.plot(z_sylvain, niz_sylvain, label = "sylvain")
#plt.plot(z, niz_davide, label = "davide")
plt.plot(z, wil_tot_davide, label = "davide")
plt.plot(wil_vincenzo_2[:1000,0], wil_vincenzo_2[:1000,i+1], label = "vincenzo")
plt.legend()
#plt.yscale("log")
