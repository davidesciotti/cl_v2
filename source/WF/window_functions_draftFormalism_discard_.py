...
z_min   = z_minus[0]
z_max   = z_plus[9]
z_max   = 4 # changed according to ssc draft yyy
...
n_bar = 30 # total number density yyy
...
def wil_tilde(z, i): #xxx attention, check carefully
    # yyy changed zprime to z in n_i(z, i)
    integrand = lambda z_prime, z, i: n_i(z, i)/n_bar * \
                (1 - r(z)/r(z_prime))
...
def wil(z,i):
    return (3/2)*(H0/c)**2 * Om0*(1+z)*r(z)*wil_tilde(z,i)

# TODO: REWRITE WFs using formalism of the draft
########################################################### IA

def W_IA(z,i):
    result = (H0/c)*(n_i(z,i)/n_bar)*E(z) # yyy added n_bar, not quite sure
    return result
...
def wig(z,i):
    return b(i)*(n_i(z,i)/n_bar_i(i))*H0*E(z)/c # xxx i divided by c
...
#wig_davide   = np.asarray([wig(zi, 0) for zi in z])
wil_SSC_davide   = np.asarray([wil_SSC(zi, 0) for zi in z])

#plt.plot(z, pph_array, label = "pph")
#plt.plot(z, nz_array,  label = "n(z)")
plt.plot(z, wil_SSC_davide, label = "davide")
plt.plot(wil_SSC_vincenzo[:,0], wil_SSC_vincenzo[:,1], label = "vincenzo")
plt.legend()
plt.yscale("log")
