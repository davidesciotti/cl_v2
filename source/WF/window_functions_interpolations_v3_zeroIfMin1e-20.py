...
n_bar = np.genfromtxt(r"%s\output\base_functions_v3\n_bar.txt" %path)
...
n_i_import = np.genfromtxt(r"%s\output\base_functions_v3\niz_e-20cut.txt" %path)
def n_i(z,i):
...
def wil_tilde(z, i): #xxx attention, check carefully
    ...
    result = quad(integrand, z, z_max, args = (z, i), limit = 500)
    return result[0]
...
def save(array, name):
    np.save(r"%s\output\WFs_v3\%s.npy" %(path, name), array)
...
# for i in range(zbins):
#     E_toplot = np.load(r"%s\output\base_functions\E(z).npy" %path)
#     plt.plot(n_i_toplot[:,0], n_i_toplot[:,i+1])
...
# np.savetxt(r"%s\output\base_functions_v3\L_ratio.txt" %path, array_appoggio)
# print("L_ratio DONE")
# array_appoggio = compute_2D(n_i, z)
# np.savetxt(r"%s\output\base_functions_v3\niz.txt" %path, array_appoggio)
# print("niz DONE")

# plt.plot(array_appoggio[:,0], array_appoggio[:,1])
# plt.yscale("log")
