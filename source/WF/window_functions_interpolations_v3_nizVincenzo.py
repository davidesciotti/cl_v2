...
n_bar = np.genfromtxt(r"%s\output\base_functions_v4\n_bar.txt" %path)
...
n_i_import = np.load(r"%s\data\Cij-NonLin-eNLA_15gen\niTab-EP10-RB00.dat" %path)
def n_i(z,i):
...
def save(array, name):
    np.save(r"%s\output\WFs_v4_nizVincenzo\%s.npy" %(path, name), array)
...
# np.savetxt(r"%s\output\base_functions_v4\L_ratio.txt" %path, array_appoggio)
# print("L_ratio DONE")
