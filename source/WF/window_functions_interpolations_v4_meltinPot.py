...
# n_i_import = np.genfromtxt(r"%s\data\Cij-NonLin-eNLA_15gen\niTab-EP10-RB00.dat" %path) # vincenzo
n_i_import = np.genfromtxt(r"%s\output\%s\niz_e-19cut.txt" %(path, WFs_folder)) # davide e-20cut
# n_i_import = np.load(r"%s\output\base_functions_v2\n_i(z,i).npy" %path) # davide standard
def n_i(z,i):
...
def wil_tilde(z, i): #xxx attention, check carefully
    ...
    result = quad(integrand, z, z_max, args = (z, i), limit = 200)
    return result[0]
...
def save(array, name):
    np.save(r"%s\output\%s\%s.npy" %(path, WFs_folder, name), array)
...
# plt.plot(lumin_ratio[:,0], lumin_ratio[:,1])
