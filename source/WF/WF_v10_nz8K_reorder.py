...
# n(z) is normalized to 30 gal/arcmin^2
...
def n(z):
    ...
    result = result*(30/0.4242640687118783) # normalized so as to have ntot = 30 gal/arcmin^2 ooo
    return result
...
############# WIL TILDE DEBUGGING ##############
# wil_tilde_array = np.zeros((zsteps, zbins+1))
# wil_tilde_array[:,0] = z


wil_tilde_array = compute_2D(wil_tilde, z)

for column in range(10):
    plt.plot(wil_tilde_array[:,0], wil_tilde_array[:,column +1])
plt.yscale("log")
