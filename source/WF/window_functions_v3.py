...
zsteps = 30
z = np.linspace(z_min, 3, zsteps)

i = 0
wil_SSC_array = np.asarray([wil_SSC(zi,i) for zi in z])
D_array   = np.asarray([D(zi) for zi in z])
niz_array = np.asarray([n_i(zi, i) for zi in z])

np.save("%s/output/wil_SSC.npy", wil_SSC_array)
np.save("%s/output/D(z).npy", D_array)
#np.save("%s/output/niz_array.npy", niz_array)


#wil_vincenzo = np.genfromtxt("%s/data/dati_vincenzo/WiL.dat" %path)
#D_vincenzo = np.genfromtxt("%s/data/dati_vincenzo/GrowthFactor.dat" %path)
#niz_vincenzo = np.genfromtxt("%s/data/dati_vincenzo/niNorm.dat" %path)
#
#plt.plot(z, wil_SSC_array, label = "davide")
#plt.plot(wil_vincenzo[:,0], wil_vincenzo[:,1])
#plt.legend()
#plt.yscale("log")
