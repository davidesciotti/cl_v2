...
###################################################
zsteps = 75
z = np.linspace(0.01, 2.1, zsteps)



def compute():
    wil_array     = np.zeros((z.shape[0], zbins))
    ...
    for i in range(zbins):
        wil_array[:,i]     = np.asarray([wil(zi, i) for zi in z])  
        ...
        print("debug: I'm working on column number %i, selector = %s" %(i, selector))
        end = time.time()
...
def insert_zColumn(wil_array, wil_tot_array, wig_array, wil_SSC_array, wig_SSC_array, niz_array, D_array):
    D_array = np.reshape(D_array, (z.shape[0], 1))
    
    wil_array     = np.insert(wil_array, 0, z, axis=1)
...
def save():
    if selector == "niz_di_davide":
        path_nightRun = "/home/davide/Scrivania/PySSC-mio/programmi/Cij_davideoutput/nightRun_nizDavide"
    elif selector == "niz_di_vincenzo":
        path_nightRun = "/home/davide/Scrivania/PySSC-mio/programmi/Cij_davideoutput/nightRun_nizVincenzo"

        np.save("%s/wil.npy"     %path_nightRun, wil_array)
        np.save("%s/wil_tot.npy" %path_nightRun, wil_tot_array)
        np.save("%s/wig.npy"     %path_nightRun, wig_array)
        np.save("%s/wil_SSC.npy" %path_nightRun, wil_SSC_array)
        np.save("%s/wig_SSC.npy" %path_nightRun, wig_SSC_array)
        np.save("%s/n_i(z).npy"  %path_nightRun, niz_array)
        np.save("%s/D(z).npy"    %path_nightRun, D_array)
        np.save("%s/z.npy"       %path_nightRun, z)
    
    
########################### computing and saving the functions

#for selector in ["niz_di_davide", "niz_di_vincenzo"]:
#    wil_array, wil_tot_array, wig_array, wil_SSC_array, wig_SSC_array, niz_array, D_array = compute()
#    insert_zColumn(wil_array, wil_tot_array, wig_array, wil_SSC_array, wig_SSC_array, niz_array, D_array)
#    save()
...
if __name__ == '__main__':
    ...
    for zi in z:
        p = multiprocessing.Process(target=wil(zi,0), args=(i,))
        processes.append(p)
        p.start()
...
print("the program took %f hours to run" %time_hours )
############ sylvain
...
#plt.plot(z, pph_array, label = "pph")
#plt.plot(z, nz_array,  label = "n(z)")
#plt.plot(z, wig_davide, label = "davide")
#plt.plot(wig_vincenzo_2[:,0], wig_vincenzo_2[:,i+1], label = "vincenzo")
#plt.legend()
#plt.yscale("log")
