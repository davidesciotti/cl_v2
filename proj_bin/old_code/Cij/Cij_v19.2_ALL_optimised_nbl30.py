...
sys.path.append("C:/Users/dscio/Documents/Lavoro/Programmi/my_module")
...
bias_selector = "oldBias"
IA_flag = "IA"
z_max   = 4 # that's the domain of vincenzo's WFs
nz = 10000
Cij_output_folder = f"Cijs_v19_ALL/Cij_WFdavide_{IA_flag}_{bias_selector}_nz{nz}" #this needs to be fixed
wil_import = np.genfromtxt(f"{path_SSC}/data/everyones_WFs/WF_REORGANIZED_MASTERFOLDER_v2/davide/nz{nz}/wil_{IA_flag}_IST_nz{nz}.txt")
wig_import = np.genfromtxt(f"{path_SSC}/data/everyones_WFs/WF_REORGANIZED_MASTERFOLDER_v2/davide/nz{nz}/wig_IST_nz{nz}.txt")

# elif i == 1: # Sylvain nz7000 noIA
#     Cij_output_folder = "Cijs_v19_ALL/Cij_WFsylvain_noIA__oldBias_nz7000"
#     z_max   = 4 
#     wil_import = np.genfromtxt("%s/data/everyones_WFs/WF_REORGANIZED_MASTERFOLDER_v2/sylvain/wil_sylvain_noIA_IST_nz7000.dat" %(path_SSC))
#     wig_import = np.genfromtxt("%s/data/everyones_WFs/WF_REORGANIZED_MASTERFOLDER_v2/sylvain/wig_sylvain_IST_nznz7000.dat" %(path_SSC))
...
winsound.Beep(freq = 440, duration = 2000)  
print("the program took %i seconds to run" %(time.time() - start))
