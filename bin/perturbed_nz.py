...
import libr.cosmo_lib as csmlb
...
def n_i_simps(z, i):
    ...
    numerator = n(z) * quad(pph, z_minus[i], z_plus[i], args=z)[0]
...
n_0 = np.asarray([n_i(z, 0) for z in z_array])
stop = time.perf_counter()
print(stop - start)

start = time.perf_counter()
n_0_s = np.asarray([n_i_simps(z, 0) for z in z_array])
stop = time.perf_counter()
print(stop - start)
