...
def compare(array_toInterpolate, reference_array):
    ...
    for i in range(zbins):
        ...
        difference[:, i+1] = np.abs( (reference_array[:, i + 1])/interpolated_array[:, i + 1] )
...
axes[0].plot(difference[:,0], difference[:,column+1], ".-", label = '$n_i(z)$ davide/$n_i(z)$ vincenzo') 
axes[0].set(xlabel='z', ylabel='$n_i(z)$ davide/$n_i(z)$ vincenzo', title = "$n_i(z)$ davide/$n_i(z)$ vincenzo")
axes[0].legend()

axes[1].plot(niz_davide[:,0], niz_davide[:,column+1], ".-", label = "davide") 
axes[1].plot(niz_vincenzo_2[:,0], niz_vincenzo_2[:,column+1], ".-", label = "vincenzo")
axes[1].set(xlabel='z', ylabel='$n_i(z)$', title = "$n_i(z)$")
axes[1].legend()

axes[2].plot(niz_davide[:,0], niz_davide[:,column+1], ".-", label = "davide")
axes[2].plot(niz_vincenzo_2[:,0], niz_vincenzo_2[:,column+1], ".-", label = "vincenzo")
axes[2].set(yscale = "log", xlabel='z', ylabel='$n_i(z)$', title = "$n_i(z)$, log scale")
