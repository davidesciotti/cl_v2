...
############################### DAVIDE ##########################################
WF_folder = "WFs_v5_ncutVincenzo"
WF_folder = "WFs_v2"
wig_davide = np.load(r"%s\output\%s\wig.npy" %(path, WF_folder))
# wig_SSC_davide = np.load(r"%s\output\%s\wig_SSC.npy" %(path, WF_folder))
wil_davide = np.load(r"%s\output\%s\wil.npy" %(path, WF_folder))
wil_tot_davide = np.load(r"%s\output\%s\wil_tot.npy" %(path, WF_folder))
# wil_SSC_davide = np.load(r"%s\output\%s\wil_SSC.npy" %(path, WF_folder))
niz_davide = np.load(r"%s\output\%s\niz.npy" %(path, WF_folder))
...
array = niz_davide
# half = np.int(niz_davide.shape[0]/2)
half = 300
for i in range(array.shape[0]):
    for j in range(zbins):
        if (array[i,j+1] < minima_left[j] and i < half):
            array[i,j+1] = 0
        if (array[i,j+1] < minima_right[j] and i > half):
            array[i,j+1] = 0            
niz_davide = array
plot(niz_davide, niz_vincenzo, niz_sylvain, column)
